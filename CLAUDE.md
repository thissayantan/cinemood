...
### Android WatchlistItem is a nested structure, not flat
**NEVER** write a flat Android `WatchlistItem` model. The server shape is `{ title: Title, status, added_at, catalog_no }` where `Title` is a nested object with `id`, `type`, `title` (string), `posterPath`, `genres`, `runtime`, `voteAverage`, `imdbRating`, `providers`, etc.
- Why: the original flat Kotlin model (with `val id: Int`, `val title: String` etc. at the top level) caused SerializationException on every screen load because `title` is a JSON object on the wire, not a string. All screens crashed silently until this was corrected.
- How to apply: `WatchlistItem.title` is always `Title` (data class). Access title fields via `item.title.id`, `item.title.title`, `item.title.runtime`, etc. `item.status`, `item.addedAt`, `item.catalogNo` stay at the top level. `Title.providers` is `JsonObject?`; extract names via the `Title.providerNames` extension (keys of the map). `RecommendResponse` has separate `recommendations: List<Recommendation>` and `items: List<WatchlistItem>` — join them by `titleId` in the ViewModel.
